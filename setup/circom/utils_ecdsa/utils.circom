...
template LimbsToBits(limb_size, num_limbs) {
    ...
    // Note that this code reverses the order of the limbs, *and* reverses 
    // the order of the bits within each limb
    for (var i = 0; i < num_limbs; i++) {
        num_to_bits[i] = Num2Bits(limb_size);
        num_to_bits[i].in <== limbs[num_limbs - i - 1];
        for (var j = 0; j < limb_size; j++) {
...
// Takes `limb_size*num_limbs` number of bits and outputs an 
// an array of `num_limbs` limbs, each of `limb_size` bits
template BitsToLimbs(limb_size, num_limbs) {
    signal input bits[limb_size*num_limbs];
    signal output limbs[num_limbs];

    assert(num_limbs > 0);
    assert(limb_size > 0);

    var N = limb_size*num_limbs;

    component bits_to_num[num_limbs];

    for (var i = 0; i < num_limbs; i++) {
        bits_to_num[i] = Bits2Num(limb_size);

        for (var j = 0; j < limb_size; j++) {
            bits_to_num[i].in[j] <== bits[(num_limbs - 1 - i)*limb_size + (limb_size - 1 - j)]; 
        }

        limbs[i] <== bits_to_num[i].out;
    }

}

template Truncate(input_bits, output_bits) {
    signal input inputs[input_bits];
    signal output outputs[output_bits];

    assert(output_bits <= input_bits);

    for(var i = 0; i < output_bits; i++) {
        outputs[i] <== inputs[i];
    }
}
