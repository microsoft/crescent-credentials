// copied from https://github.com/nalinbhardwaj/Nova-Scotia/blob/main/src/circom/file.rs
#![allow(unused_variables, dead_code)]

use ff::{PrimeField, derive::byteorder::{ReadBytesExt, LittleEndian}};
...
use crate::{circuit::Constraint, G};
...
pub fn from_reader<R: Read + Seek>(mut reader: R, header_only: bool) -> Result<R1CSFile<<G as Group>::Scalar>> {
    let mut magic = [0u8; 4];
    ...
    if header_only {
        for _ in 0..num_sections {
            ...
            if section_type == header_type {
                reader.seek(SeekFrom::Start(offset))?;
                let header = read_header(&mut reader, section_size)?;
                return Ok(R1CSFile {
                    version,
                    header,
                    constraints: Vec::new(),
        ...
        return Err(Error::new(ErrorKind::InvalidData, "Failed to find header"));
    }
    ...
    let constraints = read_constraints::<&mut R, <G as Group>::Scalar>(
        &mut reader,
...
mod tests {
    ...
    "
        ...
        let file = from_reader(reader, false).unwrap();
