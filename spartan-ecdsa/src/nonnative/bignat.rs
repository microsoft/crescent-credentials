...
impl<Scalar: PrimeField> BigNat<Scalar> {
  ...
  pub fn decompose<CS: ConstraintSystem<Scalar>>(
    ...
    let bitvectors: Vec<Bitvector<Scalar>> = self
      ...
      .collect::<Result<Vec<_>, _>>()?;
    let mut bits = Vec::<LinearCombination<Scalar>>::new();
...
mod tests {
  ...
  #[test]
  fn test_bignat_can_decompose() {
      for (n, limb_width) in [(254, 64), (254, 16), (256, 64), (256, 16), (381, 64), (381, 16)] {
  ...
  #[test]
  fn test_bignat_small_arithmetic() {
    let limb_width = 16;
    let n_limbs = 16;
    let r_bn254 = "30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001";
    let r_bls12381 = "73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001";

    for modulus in [r_bn254, r_bls12381] {
      let mut cs = TestConstraintSystem::<Scalar>::new();
      let r = BigInt::from_str_radix(&modulus, 16).unwrap();
      let a = BigInt::from(500);
      let b = BigInt::from(2);
      let c = BigInt::from(42);
      let d = BigInt::from(1042); // d = a*b + c

      let r_bn = alloc_bignat_constant(cs.namespace(||"alloc r"), &r, limb_width, n_limbs).unwrap();

      let a_bn = BigNat::alloc_from_nat(cs.namespace(||"alloc a"), ||Ok(a), limb_width, n_limbs).unwrap();
      let b_bn = BigNat::alloc_from_nat(cs.namespace(||"alloc b"), ||Ok(b), limb_width, n_limbs).unwrap();
      let c_bn = BigNat::alloc_from_nat(cs.namespace(||"alloc c"), ||Ok(c), limb_width, n_limbs).unwrap();
      let d_bn = BigNat::alloc_from_nat(cs.namespace(||"alloc d"), ||Ok(d), limb_width, n_limbs).unwrap();

      let (_, tmp) = a_bn.mult_mod(cs.namespace(||"a*b"), &b_bn, &r_bn).unwrap();
      let tmp = tmp.add(&c_bn).unwrap();
      let res = tmp.red_mod(cs.namespace(||"final reduce"), &r_bn).unwrap();
      res.equal_when_carried(cs.namespace(||"res =? c"), &d_bn).unwrap();
    
      assert!(cs.is_satisfied());
      println!("num constraints for simple bignat arithmetic: {}", cs.num_constraints());
    }

  }

  #[test]
  fn test_bignat_arithmetic() {
    let limb_width = 16;
    let n_limbs = 16;
    let r_bn254 = "30644e72e131a029b85045b68181585d2833e84879b9709143e1f593f0000001";     // 254 bits
    let r_bls12381 = "73eda753299d7d483339d80809a1d80553bda402fffe5bfeffffffff00000001";  // 255 bits, order of G1
    ...
    for modulus in [r_bn254, r_bls12381] {
      let mut cs = TestConstraintSystem::<Scalar>::new();
      let r = BigInt::from_str_radix(&modulus, 16).unwrap();
      let a = rng.gen_bigint_range(&zero, &r);
      let b = rng.gen_bigint_range(&zero, &r);
      let c = rng.gen_bigint_range(&zero, &r);
      let d = &a * &b + &c; // d = a*b + c (mod r)
      let d = d.rem(&r);
      ...
      assert!(cs.is_satisfied());
      assert_eq!(cs.num_constraints(), 1526);
    }
