  ...
  Scalar: PrimeField + PrimeFieldBits,
{
  ...
  pub fn double<CS: ConstraintSystem<Scalar>>(&self, mut cs: CS) -> Result<Self, SynthesisError> {
    //*************************************************************/
    // lambda = (Scalar::from(3) * self.x * self.x + G::A())
    //  * (Scalar::from(2)) * self.y).invert().unwrap();
    /************************************************************ */

    // Compute tmp = (Scalar::ONE + Scalar::ONE)* self.y ? self != inf : 1
    ...
    // Now compute lambda as (Scalar::from(3) * self.x * self.x + G::A()) *
    // tmp_inv
    ...
    let lambda = AllocatedNum::alloc(cs.namespace(|| "alloc lambda"), || {
      ...
      // assumes A = 0
      Ok(tmp_inv * *prod_1.get_value().get()?)
    })?;
    ...
    cs.enforce(
      ...
      |lc| lc + lambda.get_variable(),
      |lc| lc + prod_1.get_variable(), // assumes A = 0
    );
