...
impl Assignment {
  ...
  pub fn new(assignment: &[[u8; 32]]) -> Result<Assignment, R1CSError> {
    ...
    Self::new_from_scalars(assignment_scalar.unwrap())
  }

  pub fn new_from_scalars(assignment : Vec<Scalar>) -> Result<Assignment, R1CSError> {
...
impl Instance {
  ...
  /// Construct a new `Instance` from an R1CSShape
  pub fn new_from_shape<F: ff::PrimeField>(shape : &R1CSShape<F>) -> Result<Self, R1CSError> {
    let num_cons = shape.num_cons;
    let num_vars = shape.num_vars;
    let num_inputs = shape.num_io;
    ...
    let ff_to_bytes = |e: F|-> [u8;32] {
        let repr = e.to_repr();
        let e_bytes : &[u8] = repr.as_ref();
        assert!(e_bytes.len() == 32);
        let mut e_32_bytes = [0u8;32];
        for i in 0..32 {
          e_32_bytes[i] = e_bytes[i];
        }
        e_32_bytes
    };

    let ff_to_scalar =
      |tups: &[(usize, usize, F)]| -> Result<Vec<(usize, usize, Scalar)>, R1CSError> {
        let mut mat: Vec<(usize, usize, Scalar)> = Vec::new();
        for &(row, col, val_ff) in tups {
          ...
          let val = Scalar::from_bytes(&ff_to_bytes(val_ff));
          if val.is_some().unwrap_u8() == 1 {
    ...
    let A_scalar = ff_to_scalar(&shape.A);
    if A_scalar.is_err() {
      return Err(A_scalar.err().unwrap());
    }

    let B_scalar = ff_to_scalar(&shape.B);
    if B_scalar.is_err() {
      return Err(B_scalar.err().unwrap());
    }

    let C_scalar = ff_to_scalar(&shape.C);
    if C_scalar.is_err() {
